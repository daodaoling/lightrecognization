...
class Yolov3{
    ...
    public:
        ...
        static ILayer* convBnLeaky(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input,  int outch, int ksize, int s, int p, int linx);
        static ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, nvinfer1::DataType dt);
        static void APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream);
        static void doInference(IExecutionContext& context, float* input, float* output, int batchSize);
};

void Yolov3::doInference(IExecutionContext& context, float* input, float* output, int batchSize) {
...
void Yolov3::APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream) {
    // Create builder
...
ILayer* Yolov3::convBnLeaky(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input,  int outch, int ksize, int s, int p, int linx) {
...
IScaleLayer* Yolov3::addBatchNorm2d(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, std::string lname, float eps) {
...
std::map<std::string, Weights> Yolov3::loadWeights(const std::string file) {
...
ICudaEngine* Yolov3::createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, nvinfer1::DataType dt) {
    ...
    //std::map<std::string, Weights> weightMap = loadWeights("../yolov3.wts");///home/pengzhao/catkin_ws_pengzhao/src/
    //std::map<std::string, Weights> weightMap = loadWeights(home_path+"/yolo_data/yolov3.wts");//
    std::map<std::string, Weights> weightMap = loadWeights(YoloConfig::yolov3_wts_path);//
    Weights emptywts{nvinfer1::DataType::kFLOAT, nullptr, 0};
    ...
    Weights deconvwts85{nvinfer1::DataType::kFLOAT, deval, 256 * 2 * 2};
    ...
    Weights deconvwts97{nvinfer1::DataType::kFLOAT, deval, 128 * 2 * 2};
